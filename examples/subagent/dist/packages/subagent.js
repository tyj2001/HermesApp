...
function toKotlinPromptTurnList(history) {
    ...
    for (const turn of history || []) {
        list.add(Java.newInstance("com.ai.assistance.operit.core.chat.hooks.PromptTurn", resolvePromptTurnKind(turn.kind), String(turn.content ?? ""), typeof turn.toolName === "string" ? turn.toolName : null, toJavaJsonObject(isJsonObject(turn.metadata) ? turn.metadata : undefined)));
    }
    return list;
}
...
async function sendMessage(enhancedAIService, options) {
    ...
    const onToolInvocation = options.onToolInvocation
        ...
        : null;
    const stream = await enhancedAIService.callSuspend("sendMessage", options.message, null, toKotlinPromptTurnList(options.chatHistory), null, null, FunctionType.CHAT, PromptFunctionType.CHAT, false, false, false, options.maxTokens, options.tokenUsageThreshold, onNonFatalError, null, options.customSystemPromptTemplate || null, true, null, null, null, false, null, "Subagent", onToolInvocation, null, null, true);
